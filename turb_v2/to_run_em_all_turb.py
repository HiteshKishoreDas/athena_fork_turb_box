...
for i in range(np.size(R_lsh)): # np.size(nx1)):
    for j in range(np.size(nx1)):
        ...
        if B_flag:
            os.system(f'(cd para_scan{filename_turb_add(i,j)} && ./turb_compile_freya_wB)')
        ...
        os.system(f'(cd para_scan{filename_turb_add(i,j)} && sbatch job_script_turb{filename_turb_add(i,j)}.sh)')
