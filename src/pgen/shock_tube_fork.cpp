  ...
  for (int k = pmb->ks; k <= pmb->ke; ++k) {
    for (int j = pmb->js; j <= pmb->je; ++j) {
      for (int i = pmb->is; i <= pmb->ie; ++i) {
        ...
        Real temp = prim(IPR,k,j,i) / cons(IDN,k,j,i);
        Real IEN_b4 = 0.0;
        ...
        if (temp < T_cut) {
            ...
            if (temp < T_floor){
              IEN_b4 = cons(IEN,k,j,i);
            }

            Real temp_new = 0.5*(temp+T_floor);

            cons(IEN,k,j,i) += (temp_new-temp)*cons(IDN,k,j,i)/(gamma-1);

            if (temp < T_floor){

              // cons(IEN,k,j,i) += (T_floor-temp)*cons(IDN,k,j,i)/(gamma-1);

              Real dT = (prim(IPR,k,j,i)/cons(IDN,k,j,i))-T_floor;

              if (dT!=0.0){
                printf("IEN diff: %lf\n",cons(IEN,k,j,i)-IEN_b4);
                printf("New temp: %lf\n",prim(IPR,k,j,i)/cons(IDN,k,j,i));
...
void Mesh::InitUserMeshData(ParameterInput *pin) {
  ...
  printf("T_floor: %lf, T_hot: %lf, T_cut: %lf \n", T_floor, T_hot, T_cut);
