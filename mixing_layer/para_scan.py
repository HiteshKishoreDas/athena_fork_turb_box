...
import cooling_power_law.cooling_fn as cf
...
t_cool_mix = cf.tcool_calc(
    amb_rho * np.sqrt(chi_cold), np.sqrt(T_floor * T_hot), Z, Lambda_fac=Lambda_fac
)
t_cool_amb = cf.tcool_calc(amb_rho, T_hot, Z, Lambda_fac=Lambda_fac)
t_cool_cut = cf.tcool_calc(
    amb_rho / T_cut_mul, T_cut_mul * T_hot, Z, Lambda_fac=Lambda_fac
)

t_cool_Da = cf.tcool_calc(amb_rho * np.sqrt(chi_cold), 2e5, Z, Lambda_fac=Lambda_fac)

amb_rho_fix = 1.0
t_cool_cloud_fix = cf.tcool_calc(
    amb_rho_fix * chi_cold, T_floor, Z, Lambda_fac=Lambda_fac
)
...
box_width = 0.5 * 0.1 * np.array([10000.0, 100.0, 1.0, 0.001, 0.0001])
