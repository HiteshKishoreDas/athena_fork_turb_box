...
for i in [1, 2]:  # range(np.size(R_lsh)):
    for j in range(np.size(nx1)):
        ...
        os.system(f"cp -r template_dir_cloud para_scan" + filename_cloud_add(i, j))

        # os.system(f'rm para_scan'+filename_cloud_add(i,j)+'/job_script_turb_template.sh')
        # os.system(f'rm para_scan'+filename_cloud_add(i,j)+'/athinput.turb_v2_turb_template')
