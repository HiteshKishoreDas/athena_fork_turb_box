...
import own_package.cooling.cooling_fn as cf
...
R_dict = {}
R_dict[5] = 4.305532167256101e-06
R_dict[10] = 8.611064334512203e-06
R_dict[50] = 4.305532167256102e-05
R_dict[100] = 8.611064334512204e-05
R_dict[250] = 0.00021527660836280507
R_dict[500] = 0.00043055321672561014
R_dict[1000] = 0.0008611064334512203
R_dict[2500] = 0.0021527660836280506
R_dict[5000] = 0.004305532167256101
...
t_cool_cloud = cf.tcool_calc(rho=amb_rho * cloud_chi_temp, T=T_floor, Zsol=Zsol)

t_cool_mix = cf.tcool_calc(
    rho=amb_rho * np.sqrt(cloud_chi_temp), T=np.sqrt(T_floor * T_hot), Zsol=Zsol
)

t_cool_amb = cf.tcool_calc(rho=amb_rho, T=T_hot, Zsol=Zsol)
t_cool_cut = cf.tcool_calc(rho=amb_rho / T_cut_mul, T=T_cut_mul * T_hot, Zsol=Zsol)


l_sh = vt.cs_calc(T_floor, mu) * t_cool_cloud

R_lsh_real_dict = R_dict.copy()
for k in R_dict.keys():
    R_lsh_real_dict[k] = R_dict[k] / l_sh

R_arr = np.zeros_like(R_lsh, dtype=float)
for i in range(len(R_lsh)):
    R_arr[i] = R_dict[R_lsh[i]]

cloud_radius_temp = R_arr  # R_lsh * l_sh

# L_box = cloud_radius_temp * 40
L_box = cloud_radius_temp * 20
...
if B_flag:
    ...
    # multi = 11.4/8.0
    # dedt  = multi*vt.dedt_calc_mhd(M,amb_rho,T_hot,L_box)

    multi = 2
    dedt = vt.dedt_calc_hydro(M, amb_rho, T_hot, L_box) * multi

else:
    # dedt  = vt.dedt_calc_hydro(M,amb_rho,T_hot,L_box)
    dedt = vt.dedt_calc_hydro(M, amb_rho, T_hot, L_box)
...
v_turb_predict = M * vt.cs_calc(T_hot, mu)
# v_turb_predict = np.loadtxt('beta_last_vturb')
...
if B_flag:
    ...
    P_th = (T_hot / (g.KELVIN * g.mu)) * amb_rho
    P_B = P_th / beta_list
...
def filename_cloud_add(i, j):
    ...
    if B_flag:
        return f"_Rlsh{i}_{R_lsh[i]}_res{j}_{nx1[j]}_rseed_{rseed}_M_{M}_fshear_{f_shear}_chi_{cloud_chi}_beta_{beta_list}"
    else:
        return f"_Rlsh{i}_{R_lsh[i]}_res{j}_{nx1[j]}_rseed_{rseed}_M_{M}_fshear_{f_shear}_chi_{cloud_chi}_hydro"


def filename_cloud_func(i, j, rseed, Mach, cloud_chi, beta, MHD_flag, fshear=0.3):

    if MHD_flag:
        return f"_Rlsh{i}_{R_lsh[i]}_res{j}_{nx1[j]}_rseed_{rseed}_M_{Mach}_fshear_{fshear}_chi_{cloud_chi}_beta_{beta}"
    else:
        return f"_Rlsh{i}_{R_lsh[i]}_res{j}_{nx1[j]}_rseed_{rseed}_M_{Mach}_fshear_{fshear}_chi_{cloud_chi}_hydro"
